...
function sumLargeInt(x, y) {
    var res = [];
    var arrX = x.split("");
    var arrY = y.split("");
    var _a = arrX.length > arrY.length ? [arrX, arrY] : [arrY, arrX], largerArr = _a[0], smallerArr = _a[1];
    var len = largerArr.length;
    var diff = len - smallerArr.length;
    while (--diff + 1) {
        smallerArr.unshift("0");
    }
    var carry = 0;
    while (--len + 1) {
        var sum = Number(largerArr[len]) + Number(smallerArr[len]) + carry;
        if (sum < 10) {
