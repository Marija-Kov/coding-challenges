...
function addTwoNumArr(l1: number[], l2: number[]):number[] {
 let num1: number = Number(l1.reverse().join(''))
 let num2: number = Number(l2.reverse().join(""));
...
function findNextSquare(sq: number): number {
    ...
    let diff: number = sqrt - Math.floor(sqrt)
  if(!diff) return (sqrt+1)**2
  return -1;
}
...
function getBestWord(points: number[], words: string[]){
 let len = points.length;
 let letterPts: Points = {};
 for(let i=0; i<len; ++i){
 ...
 function sumPts(word: string, index: number): number[]{
    let len = word.length;
    let sum = 0;
    for(let i=0; i<len; ++i){
...
function genRandomStrArr(n: number, l: number): string[] {
  let alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  let arr = [];
   for(let i=0; i<n; ++i){
    arr.push(randomStr())
